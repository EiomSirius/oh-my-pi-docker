...
# omp/bun accesibles para el usuario omp tras recreates (idempotente)
ln -sf /usr/local/bun/bin/bun /usr/local/bin/bun 2>/dev/null || true
ln -sf /usr/local/bun/bin/omp /usr/local/bin/omp 2>/dev/null || true
